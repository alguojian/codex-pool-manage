      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 overflow-y-auto p-6 max-w-3xl">
          ...
          <section className="space-y-4 mb-8">
            ...
            {[
              { key: 'accountErrors', label: t('settings.accountErrors') },
              { key: 'rateLimitsHit', label: t('settings.rateLimitsHit') },
              { key: 'taskFailures', label: t('settings.taskFailures') },
              { key: 'poolRotation', label: t('settings.poolRotation') }
            ].map(({ key, label }) => (
          ...
          <section className="space-y-3 mb-8">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => toast.info('Import accounts API not implemented yet')}>{t('settings.importAccounts')}</Button>
              <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => toast.success('Export from database is ready to add next')}>{t('settings.exportAccounts')}</Button>
              <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => toast.info('Import config API not implemented yet')}>{t('settings.importPoolConfig')}</Button>
              <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => toast.success('Current settings are persisted in MySQL')}>{t('settings.exportPoolConfig')}</Button>
            </div>
          </section>
          ...
          <section className="space-y-3 rounded-lg border border-destructive/30 p-4">
            ...
            <div className="flex gap-2 flex-wrap">
              <Button variant="outline" size="sm" className="h-8 text-xs border-destructive/30 text-destructive hover:bg-destructive/10" onClick={() => toast.error('Bulk reset endpoint can be added next')}>{t('settings.resetAllAccounts')}</Button>
              <Button variant="outline" size="sm" className="h-8 text-xs border-destructive/30 text-destructive hover:bg-destructive/10" onClick={() => toast.error('Task history cleanup endpoint can be added next')}>{t('settings.clearTaskHistory')}</Button>
              <Button variant="outline" size="sm" className="h-8 text-xs border-destructive/30 text-destructive hover:bg-destructive/10" onClick={() => toast.error('Full data wipe endpoint is intentionally not wired yet')}>{t('settings.deleteAllData')}</Button>
            </div>
          </section>
        </div>
      </div>
